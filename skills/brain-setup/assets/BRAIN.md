...
| `normalize_timestamps` | `brain normalize-timestamps [--dry-run]` — migrates `created`, `updated`, and timeline `time` values to second precision (`YYYY-MM-DDTHH:MM:SS`) and reindexes unless `--dry-run` is used. |
